...
@Component({
  selector: 'app-policy-scenarios',
  templateUrl: './policy-scenarios.component.html',
  styleUrls: ['./policy-scenarios.component.css']
})
export class PolicyScenariosComponent {


appliedPoliciesReceived :any = []
policyScenariosReceived: any = []
...
console.log(this.caseStudyName);
this.policyRecommendation.getAssesmentAndStoryline(this.caseStudyName)
  .subscribe((data: any[]) => {
    this.policyScenariosReceived = data;
    console.log('received:', this.policyScenariosReceived);
  });    console.log(this.policyScenariosReceived);
...
execute_button:boolean  = true;
  ...
  checkInputValue(inputSelected:string,index:number){
    this.thePolicySelected.setPolicySelected(inputSelected,index);
    console.log(index);
  ...
  this.policyRecommendation.setSelectedScenarioCode(value); // <-- store in service
}
}
